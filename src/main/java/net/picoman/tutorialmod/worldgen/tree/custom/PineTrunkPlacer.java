package net.picoman.tutorialmod.worldgen.tree.custom;
...
public class PineTrunkPlacer extends TrunkPlacer {
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> pBlockSetter,
        ...
        setDirtAt(pLevel, pBlockSetter, pRandom, pPos.below(), pConfig); //pour placer la sapling sur de la dirt
        int height = pFreeTreeHeight + pRandom.nextInt(heightRandA, heightRandA + 3) + pRandom.nextInt(heightRandB - 1, heightRandB + 1);

        for(int i = 0; i < height; i++) {
            placeLog(pLevel, pBlockSetter, pRandom, pPos.above(i), pConfig); //place des bûches verticales jusqu'à atteindre la hauteur générée

            if(i % 2 == 0 && pRandom.nextBoolean()) { //pRandom.nextBoolean ajoute une chance de 50%
        ...
        return ImmutableList.of(new FoliagePlacer.FoliageAttachment(pPos.above(height), 0, false)); //aide si on doit mettre du feuillage sur des branches et pas uniquement en haut
    }
}
