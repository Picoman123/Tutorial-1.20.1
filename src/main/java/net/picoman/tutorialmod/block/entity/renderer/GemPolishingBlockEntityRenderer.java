...
package net.picoman.tutorialmod.block.entity.renderer;
...
public class GemPolishingBlockEntityRenderer implements BlockEntityRenderer<GemPolishingStationBlockEntity> {
    ...
    public void render(GemPolishingStationBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack,
        ...
        pPoseStack.pushPose();
        pPoseStack.translate(0.5f, 0.75f, 0.5f); //bouge l'item qui va être render vers le milieu du bloc
        pPoseStack.scale(0.35f, 0.35f, 0.35f); //prend 35% de la taille normale
        pPoseStack.mulPose(Axis.XP.rotationDegrees(270)); //rotation de 270°
    ...
    private int getLightLevel(Level level, BlockPos pos) { //pour s'assurer que la brightness de l'item est correcte
