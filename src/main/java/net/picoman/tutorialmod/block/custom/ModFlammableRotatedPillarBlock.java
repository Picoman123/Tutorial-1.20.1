...
public class ModFlammableRotatedPillarBlock extends RotatedPillarBlock {
    ...
    public boolean isFlammable(BlockState state, BlockGetter level, BlockPos pos, Direction direction) {
        return true; //le bois est flammable
    }
    ...
    public @Nullable BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction toolAction, boolean simulate) {
        if(context.getItemInHand().getItem() instanceof AxeItem) {
            if(state.is(ModBlocks.PINE_LOG.get())) {
                return ModBlocks.STRIPPED_PINE_LOG.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS)); //il faut préciser la valeur de l'axe pour pas avoir du stripped dans le mauvais sens
            }
