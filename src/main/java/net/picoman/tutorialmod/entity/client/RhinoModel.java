package net.picoman.tutorialmod.entity.client;

import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.vertex.VertexConsumer;
import net.picoman.tutorialmod.entity.animations.ModAnimationDefinitions;
...
public class RhinoModel<T extends Entity> extends HierarchicalModel<T> { //passer en hierachicalModel au lieu de Entitymodel car plus facile pour les animations
	private final ModelPart rhino;
	private final ModelPart head;

	public RhinoModel(ModelPart root) {
		this.rhino = root.getChild("rhino");
		this.head = rhino.getChild("body").getChild("torso").getChild("head"); //voir ordre des fichiers dans BlockBench (rhino, body, torso, head)
	}
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.root().getAllParts().forEach(ModelPart::resetPose); //si on met pas ça les animations se superposent
	...
	private void applyHeadRotation(float pNetHeadYaw, float pHeadPitch, float pAgeInTicks) { //pour que la tête se tourne vers le joueur quand le rhino le regarde
